...
args = commandArgs()
f = args[4]
of = paste0(f, '-dep-mutual-new1')
load("dp.crispr.19q1.rda")
load("dp.rnai.19q1.rda")
gt = alias2SymbolTable(dp.rnai.19q1$genes) #
sl = fread(f, head=T)
...
pcl=NULL
for (k in seq(2)){
	if (k==1) probc=dp.crispr.19q1
	if (k==2) probc=dp.rnai.19q1
	mRNAq=probc$mRNAq2
	scnaq=probc$scnaq2
	mat=probc$mat
